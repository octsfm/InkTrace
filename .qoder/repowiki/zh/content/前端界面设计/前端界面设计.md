...
H["views/novel/NovelImport.vue"]
I["views/character/CharacterManage.vue"]
J["views/worldview/WorldviewManage.vue"]
K["views/config/LLMConfig.vue"]
end
subgraph "状态与API"
L["stores/config.js<br/>Pinia配置状态"]
M["api/index.js<br/>通用API封装"]
N["api/config.js<br/>配置API客户端"]
end
A --> B
A --> C
A --> D
...
- [frontend/src/views/novel/NovelList.vue:1-215](file://frontend/src/views/novel/NovelList.vue#L1-L215)
- [frontend/src/views/novel/NovelDetail.vue:1-432](file://frontend/src/views/novel/NovelDetail.vue#L1-L432)
- [frontend/src/views/novel/NovelImport.vue:1-269](file://frontend/src/views/novel/NovelImport.vue#L1-L269)
...
- [frontend/src/api/index.js:1-171](file://frontend/src/api/index.js#L1-L171)
...
### 小说导入：NovelImport
- 功能：小说导入界面，支持文件选择、大纲文件上传、实时进度跟踪、增强的用户反馈系统。
- 新增特性：
  - 实时进度跟踪：使用el-steps组件显示导入四个阶段的进度
  - 增强用户反馈：ElMessage提供多种类型的提示信息
  - Electron集成：支持桌面应用的文件选择对话框
  - 多种导入方式：支持Electron原生文件选择和浏览器文件上传
- 数据流：表单验证 -> 创建小说 -> 解析文件 -> 整理结构 -> 完成导入 -> 跳转详情页

```mermaid
sequenceDiagram
participant U as "用户"
participant NI as "NovelImport.vue"
participant NA as "novelApi"
participant CA as "contentApi"
U->>NI : 打开导入界面
NI->>NI : 表单验证
U->>NI : 点击"开始导入"
NI->>NA : POST /novels/ 创建小说
NA-->>NI : 返回小说ID
NI->>NI : 显示进度步骤1："创建项目"
NI->>CA : POST /content/import 或 /content/import/upload
CA-->>NI : 导入完成
NI->>NI : 显示进度步骤2："解析文件"
NI->>NI : 显示进度步骤3："整理结构"
NI->>NI : 显示进度步骤4："完成"
...
**图表来源**
- [frontend/src/views/novel/NovelImport.vue:170-230](file://frontend/src/views/novel/NovelImport.vue#L170-L230)
- [frontend/src/api/index.js:97-107](file://frontend/src/api/index.js#L97-L107)
...
ImportPage["NovelImport.vue"] --> API
```
...
- 常见问题定位：
  ...
  - 导入失败：检查文件格式、大小限制、磁盘空间，查看控制台错误信息。
...
## 结论
InkTrace前端采用清晰的分层架构与模块化设计，借助Vue 3与Element Plus实现高效开发，配合Pinia与Axios形成稳定的状态与数据流。通过路由与布局解耦、API封装与错误处理统一、组件间职责明确，整体具备良好的可维护性与扩展性。新增的小说导入界面增强了用户体验，提供了实时进度跟踪和丰富的用户反馈机制。建议后续持续完善单元测试、接入TypeScript与ESLint/Prettier规范，进一步提升质量与协作效率。
...
- 文件上传：支持multipart/form-data格式的文件上传，包括普通文件和大纲文件。

**章节来源**
- [frontend/src/api/index.js:4-41](file://frontend/src/api/index.js#L4-L41)
- [frontend/src/api/index.js:43-171](file://frontend/src/api/index.js#L43-L171)
...
- [frontend/src/views/novel/NovelImport.vue:75-85](file://frontend/src/views/novel/NovelImport.vue#L75-L85)
...
- Electron集成：通过window.electronAPI检测桌面应用环境，提供原生功能支持。
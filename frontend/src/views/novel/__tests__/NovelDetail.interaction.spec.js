...
const mockNovelListChapters = vi.fn()
...
vi.mock('@/api', () => ({
  novelApi: {
    ...
    listChapters: (...args) => mockNovelListChapters(...args),
    deleteChapter: vi.fn()
  },
  projectApi: {
    getByNovel: (...args) => mockGetByNovel(...args),
    memoryV2: (...args) => mockMemory(...args),
...
describe('NovelDetail 展示与整理进度', () => {
  beforeEach(() => {
    ...
    mockNovelListChapters.mockResolvedValue([{ id: 'c1', number: 1, title: '第一章', status: 'saved', updated_at: '2026-01-01' }])
  ...
  it('整理完成后只刷新一次详情数据，不递归重载页面', async () => {
    ...
    expect(mockNovelGet).toHaveBeenCalledTimes(1)
    expect(mockGetByNovel).toHaveBeenCalledTimes(1)
    expect(mockMemoryView).toHaveBeenCalledTimes(1)
    expect(mockActivePlotArcsV2).toHaveBeenCalledTimes(1)
  ...
  it('优先使用独立章节接口和记忆数据展示章节与三层剧情摘要', async () => {
    mockNovelGet.mockResolvedValue({
      id: 'novel_1',
      title: '测试小说',
      author: '作者',
    ...
    mockNovelListChapters.mockResolvedValue([
      { id: 'c1', number: 1, title: '开局', status: 'saved', updated_at: '2026-01-01' },
      { id: 'c2', number: 2, title: '遇敌', status: 'saved', updated_at: '2026-01-02' },
      { id: 'c3', number: 3, title: '破局', status: 'draft', updated_at: '2026-01-03' }
    ])
    mockMemory.mockResolvedValue({
      chapter_summaries: [
        '开局：主角意外卷入修真世界。',
      ...
      current_state: {
        latest_summary: '主角已经与敌对势力正式对上。',
        next_writing_focus: '继续推进苍茫山的冲突。'
      }
    })
    mockMemoryView.mockResolvedValue({
      current_progress: '主角已进入苍茫山冲突阶段。',
      main_plot_lines: ['主角不断逼近苍茫山真相。'],
      outline_summary: ['从都市修真开局逐步推进到宗门冲突。']
    })
    mockOrganizeProgress.mockResolvedValue({ status: 'done', stage: 'done', current: 3, total: 3, percent: 100, current_chapter_title: '破局', message: '整理完成' })
    ...
    expect(mockNovelListChapters).toHaveBeenCalledWith('novel_1', expect.any(Object))
    expect(text).toContain('开局')
    expect(text).toContain('破局')
    expect(text).toContain('从主角意外卷入修真世界。起步')
    expect(text).toContain('从开局的主角意外卷入修真世界。')
    expect(text).toContain('遇敌：主角第一次正面碰撞敌对势力。')
  })

  it('鏁寸悊澶辫触鏃朵細鍙彁绀轰竴娆℃槑纭殑閿欒 toast', async () => {

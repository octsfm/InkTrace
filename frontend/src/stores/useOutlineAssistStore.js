...
export const useOutlineAssistStore = defineStore('workbenchOutlineAssist', () => {
  ...
  const isSelectionOnlySuggestion = (item) => {
    ...
    if (!['outline_polish', 'outline_expand'].includes(suggestionType)) return false
  ...
  const isAcceptedWritingTaskSuggestion = (item) => String(item?.suggestion_type || '') === 'writing_task_suggestion' && getSuggestionStatus(item) === 'accepted'
  const isStaleSuggestion = (item) => String(item?.status || '').toLowerCase() === 'stale'
  const canAcceptSuggestion = (item) => !isPendingLifecycleSuggestion(item) && !isTerminalSuggestion(item) && getSuggestionStatus(item) !== 'accepted'
  const canResolveSuggestion = (item) => !isPendingLifecycleSuggestion(item) && !isTerminalSuggestion(item) && !isAcceptedWritingTaskSuggestion(item)
  const canConvertSuggestion = (item) => canResolveSuggestion(item) && getSuggestionStatus(item) !== 'accepted' && !['risk_warning', 'writing_task_suggestion'].includes(String(item?.suggestion_type || ''))
  const canApplySuggestion = (item) => OUTLINE_APPLYABLE_SUGGESTION_TYPES.has(String(item?.suggestion_type || '')) && getSuggestionStatus(item) === 'accepted' && !isSelectionOnlySuggestion(item)

  const filteredSuggestions = computed(() => suggestions.value.filter((item) => normalizeSuggestionMode(item?.suggestion_type) === activeMode.value))
  ...
  const ensureAvailableMode = () => {
    ...
    const firstAvailableMode = modes.find((mode) => suggestions.value.some((item) => normalizeSuggestionMode(item?.suggestion_type) === mode.id))?.id
  ...
  const loadSuggestions = async ({ workId: targetWorkId = workId.value, chapterId: targetChapterId = '' } = {}) => {
    ...
    loading.value = true
    try {
      const payload = await aiApi.listAISuggestions({ work_id: String(targetWorkId || ''), chapter_id: nextChapterId })
  ...
  const loadSuggestionDetail = async (suggestionId) => {
    ...
    suggestionDetails.value = { ...suggestionDetails.value, [String(suggestionId || '')]: detail }
    return detail
  }
  ...
  const isCurrentSuggestionContext = (targetWorkId = workId.value, targetChapterId = chapterId.value) => String(workId.value || '') === String(targetWorkId || '') && String(chapterId.value || '') === String(targetChapterId || '')
  ...
  const acceptSuggestion = async (suggestionId) => {
    ...
    try {
      ...
      if (!isCurrentSuggestionContext(actionWorkId, actionChapterId)) return accepted
      const acceptedId = String(accepted?.suggestion_id || suggestionId)
      suggestions.value = suggestions.value.map((item) => String(item?.suggestion_id || '') === acceptedId ? { ...item, ...accepted } : item)
  ...
  const dismissSuggestion = async (suggestionId) => {
    ...
    try {
      ...
      if (!isCurrentSuggestionContext(actionWorkId, actionChapterId)) return dismissed
      const dismissedId = String(dismissed?.suggestion_id || suggestionId)
      suggestions.value = suggestions.value.map((item) => String(item?.suggestion_id || '') === dismissedId ? { ...item, ...dismissed } : item)
      await refreshCurrentSuggestions()
      return dismissed
    } finally {
      submittingSuggestionId.value = ''
  ...
  const convertSuggestion = async (suggestionId) => {
    ...
    try {
      ...
      if (!isCurrentSuggestionContext(actionWorkId, actionChapterId)) return converted
      const convertedId = String(converted?.suggestion_id || suggestionId)
      suggestions.value = suggestions.value.map((item) => String(item?.suggestion_id || '') === convertedId ? { ...item, ...converted } : item)
      await refreshCurrentSuggestions()
      return converted
    } finally {
      submittingSuggestionId.value = ''
  ...
  const applySuggestion = async (suggestionId) => {
    if (!featureEnabled.value || !suggestionId || applySubmittingSuggestionId.value === suggestionId) return null
    const targetSuggestion = suggestions.value.find((item) => String(item?.suggestion_id || '') === String(suggestionId || ''))
    ...
    try {
      ...
      if (!isCurrentSuggestionContext(actionWorkId, actionChapterId)) return applied
      const appliedId = String(applied?.suggestion_id || suggestionId)
      suggestions.value = suggestions.value.map((item) => String(item?.suggestion_id || '') === appliedId ? { ...item, ...applied } : item)
      applyConfirmSuggestionId.value = ''
      await refreshCurrentSuggestions()
      return applied
    } catch (error) {
  ...
  const syncConflictHandoff = async (actionPayloadRef, { workId: targetWorkId = workId.value, chapterId: targetChapterId = chapterId.value } = {}) => {
    ...
    try {
      const [detailPayload, listPayload] = await Promise.all([
        ...
        aiApi.listConflicts({ work_id: String(targetWorkId || ''), chapter_id: String(targetChapterId || '') })
      ])
  ...
  const convertSuggestionWithConflictSync = async (suggestionId, { workId: targetWorkId = workId.value, chapterId: targetChapterId = chapterId.value } = {}) => {
    ...
    if (!isCurrentSuggestionContext(targetWorkId, targetChapterId)) return converted
    await syncConflictHandoff(converted?.action?.action_payload_ref, { workId: targetWorkId, chapterId: targetChapterId })
    return converted
  }
  ...
  const openApplyConfirm = (suggestionId) => { clearActionError(); applyConfirmSuggestionId.value = String(suggestionId || '') }
  const closeApplyConfirm = () => { if (!applySubmittingSuggestionId.value) applyConfirmSuggestionId.value = '' }
  const startApplySubmitting = (suggestionId) => { clearActionError(); applySubmittingSuggestionId.value = String(suggestionId || '') }

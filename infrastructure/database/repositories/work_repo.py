...
class WorkRepo:
    ...
    def list_all(self) -> List[Work]:
        with get_connection() as conn:
            rows = conn.execute(
                "SELECT id, title, author, word_count, created_at, updated_at FROM works ORDER BY updated_at DESC"
            ).fetchall()
    ...
    def find_by_id(self, work_id: str) -> Optional[Work]:
        with get_connection() as conn:
            row = conn.execute(
                "SELECT id, title, author, word_count, created_at, updated_at FROM works WHERE id = ?",
                (str(work_id),),
            ).fetchone()
    ...
    def save(self, work: Work) -> None:
        with get_connection() as conn:
            self._save_with_connection(conn, work)

    def _save_with_connection(self, conn, work: Work) -> None:
        conn.execute(
            """

...
4. 落地 AI Settings 完整闭环：双模型 Key、关键分工校验、连接测试、阻断提示、脱敏显示。
5. 形成可验收、可回归、可观测的 P1 交付版本。
...
3. 每阶段交付时同步提供最小调试入口或内部 API 验证入口：
   - S11a 统一 11 组 API 收口；
   - S11b 统一前端首批入口打通。
4. AgentTrace 基础字段从 S1 开始贯穿预留：
   - `request_id`
   - `trace_id`
   - `session_id`
   - `step_id`
...
| S11a | API集成边界 | 11组API收口、三重门控、错误码、Settings 完整闭环 API | 是 |
| S11b | 前端集成闭环 | 三栏入口、轮询主路径、SSE回退、Settings 作者路径闭环 | 是 |
| S12 | E2E联调与封板验收 | 全链路回归、封板报告、P2入口判断 | 是 |
...
强制验收（正向 + 红线）：
1. 四条正向路径均有自动化用例（approve/edit_and_approve/reject/defer）。
...
2. 安全红线回归报告。
3. AI Settings E2E 闭环回归：
   - 未配置
   - 配置失败
   - 配置成功
   - 主链路可执行
4. 输出 P1 开发完成验收报告，确认是否进入 P2 设计/开发。
...
4. API 与前端必须分别提供阶段测试证据，并在 S12 汇总闭环矩阵。
...
| ConflictGuard 审批软提示与 apply硬阻断被混淆 | 审批体验劣化、流程错误阻断 | S8/S9 联调前专项设计对齐会，冻结两阶段调用时机与返回处理 | 是 |
...
| AI Settings 仅做入口不做闭环 | AI 实际不可用 | S11a/S11b 明确闭环交付与阻断验收 | 是 |
...
| 里程碑 | 阶段范围 | 出口标准 |
|---|---|---|
| M1 运行内核可用 | S0~S3 | 各阶段强制验收项全部通过、无 blocker 缺陷、阶段验收简报已提交 |
| M2 创作主链路可用 | S4~S6 | 主路径与关键反例用例全部通过、无 blocker 缺陷、阶段验收简报已提交 |
| M3 治理与安全可用 | S7~S10 | 建议/冲突/记忆/追踪红线回归通过、无 blocker 缺陷、阶段验收简报已提交 |
| M4 产品集成可用 | S11a~S12 | API+前端+E2E 全量通过、AI Settings 闭环通过、P1 验收报告完成 |
...
1. 本计划可直接执行，阶段依赖、交付范围和验收标准已量化到可测试级别。
2. 若任一红线被触发，必须停止并回到文档裁决流程。
3. 开发过程中仅允许在不改变冻结业务语义前提下细化实现参数。
4. S11a/S11b 的 AI Settings 闭环属于 P1 必须交付，不得降级为“仅有入口”。

...
- 仅作用于 `work_outlines.content_text` 的前端编辑草稿。
- 不作用于 `chapter_outlines`。
- 不作用于章节正文、章节列表、时间线、伏笔、人物卡。
- 不新增后端解析 API。
...
- 文件读取、粘贴、字符统计、预览、覆盖、追加全部由前端完成。
- `.txt` 按纯文本读取。
- `.md` 按纯文本原样读取，不渲染 Markdown，不解析 Markdown，不生成 `content_tree_json`。
- 预览区只显示纯文本，必须进行文本转义，禁止执行 HTML 或脚本。
- 导入文件大小上限为 2MB。
- 支持编码：`utf-8-sig`、`utf-8`。
- 不支持的编码必须提示失败，不做自动转码。
- 字符统计使用原始字符数，包含空格、换行、制表符，不复用正文 `word_count` 口径。
- 20,000 字符以上显示性能建议提示。
- 50,000 字符以上显示强提示，但不阻断导入。

Merge:

- 当前作品大纲草稿为空时，导入内容直接写入草稿。
- 当前作品大纲草稿非空时，用户必须选择覆盖或追加。
- 默认导入方式为覆盖当前草稿。
- 追加时，若原草稿与导入文本均非空，中间插入 `\n\n`。

State:

- 导入结果写入 OutlinePanel 当前作品大纲 `localContent`。
- 导入后立即进入既有 dirty 状态链路。
- 导入后必须写入既有作品大纲本地暂存。
- 禁止新增 `outlineImportDraft` 状态。
- 导入后不调用 `PUT /api/v1/works/{work_id}/outline`。
- 用户点击保存作品大纲后，继续使用现有 `PUT /api/v1/works/{work_id}/outline`。
- 保存成功后更新 `version` 并清理对应本地暂存。
- 保存返回 409 时保留本地草稿、dirty 状态与本地暂存，打开结构化资产冲突处理入口。
...
- 打开/关闭不主动改变正文焦点。
- 移动端使用覆盖层。
- Drawer 头部显示当前模块标题与关闭按钮。
- Drawer 内不再重复渲染 `outline / timeline / foreshadow / character` 切换按钮。
- 模块切换统一由 `AssetRail` 触发。
- 内容区独立滚动，状态与保存操作区吸附在 Drawer 底部。
- Drawer 打开后，`AssetRail` 必须隐藏当前激活模块入口。
- 当前激活模块名称仅由 Drawer Header 展示。
...
- `AssetRail` 只显示单字入口，完整名称由 Drawer 标题承担。
- `AssetRail` 仅显示未激活模块入口。
- 当前激活入口不在 `AssetRail` 中渲染。
- 当前激活模块不通过 `AssetRail` 再次点击关闭。
- 切换模块前必须检查当前模块 dirty 状态。
- dirty 时仅允许：保存 / 放弃 / 取消。
...
- 同一时间只展示一个编辑区。
- 使用文本编辑区作为主视图。
- `content_tree_json` 仅作为派生缓存，不在 V1.1 写作页中提供独立树视图入口。
...
- 默认进入“作品大纲”。
- 切换到“章节大纲”后，仅展示当前激活章节对应的细纲编辑区。
- 不允许在同一抽屉中上下同时展示“作品大纲”和“章节大纲”，避免小高度下内容被截断。
- 仅“作品大纲”模式显示导入入口。
- “章节大纲”模式禁止显示导入入口。
- 导入入口打开 `OutlineImportModal.vue`。
- 导入完成后不保存数据库，仅写入作品大纲当前草稿并进入 dirty 状态。
- 作品大纲导入不新增 Store 状态链路，复用既有结构化资产本地暂存。
...
### 6.6 `OutlineImportModal.vue`

职责：

- 读取 TXT 文件。
- 读取 Markdown 文件并按纯文本原样展示。
- 接收用户手动粘贴的大纲文本。
- 展示纯文本预览与原始字符数。
- 将导入内容以覆盖或追加方式写入作品大纲草稿。

输入规则：

- 文件类型仅允许 `.txt`、`.md`。
- 文件大小上限 2MB。
- 编码仅支持 `utf-8-sig`、`utf-8`。
- Markdown 不渲染为 HTML，不解析标题层级，不生成树结构。
- 预览区必须转义文本内容，禁止执行 HTML 或脚本。

导入规则：

- 当前作品大纲草稿为空时，直接导入。
- 当前作品大纲草稿非空时，必须提供覆盖与追加选项。
- 默认选项为覆盖当前草稿。
- 追加时，若原草稿与导入文本均非空，中间插入两个换行。
- 点击“导入到草稿”后关闭弹窗，写入 `OutlinePanel.localContent`，标记 dirty，并写入既有本地暂存。
- 点击“导入到草稿”不得调用保存 API。
...
- 不处理章节正文导入。
- 不创建章节。
- 不创建或更新 Timeline、Foreshadow、Character。
- 不调用 AI 分析、生成、抽取能力。

### 6.7 `TimelinePanel.vue`
...
### 6.10 `WritingPreferencePanel.vue`

职责：

- 承载字体、字号、行距、主题等本地写作偏好。
- 仅修改本地偏好 Store，不触发章节保存。
- 作为 Header 按钮的悬浮面板展示。

展示规则：

- 桌面端采用锚定按钮的浮层卡片，不以内联整块方式撑高 Header。
- 移动端采用底部弹层或右侧覆盖层。
- 面板支持点击空白关闭和 `Esc` 关闭。

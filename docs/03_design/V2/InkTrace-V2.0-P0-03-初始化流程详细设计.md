...
初始化流程只提交 `model_role`，实际 provider / model 由 P0-01 ModelRouter 根据 ModelRoleConfig 与 AI Settings 决定。

P0 默认口径：

- 大纲分析使用 `model_role = outline_analyzer`，默认 Provider 倾向 Kimi。
- 正文分析使用 `model_role = manuscript_analyzer`，默认 Provider 倾向 Kimi。
- 结构化摘要、章节摘要、人物 / 剧情抽取如使用模型，应使用 `model_role = memory_extractor` 或 `manuscript_analyzer`，默认 Provider 倾向 Kimi。
- InitializationApplicationService、OutlineAnalysisService、ManuscriptAnalysisService 不得硬编码 Kimi / DeepSeek，也不得直接调用具体 Provider SDK。
- 默认 Provider 倾向只存在于 P0-01 ModelRoleConfig / AI Settings 中，用户可以在 AI Settings 中调整。
- 本节只统一默认路由说明，不改变初始化 Job、partial_success、retry / resume、stale / reanalysis 规则。
...
| ModelRoleConfig 缺失 / model_role 未配置 | model_role_invalid | 对应 Step failed；内部可记录 model_role_config_missing 作为 debug_ref | 不影响 |

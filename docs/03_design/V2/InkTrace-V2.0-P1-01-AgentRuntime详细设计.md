...
合并说明：本文档以 `_001` 版本为主稿，吸收原始版本中仍有价值但 `_001` 未充分覆盖的内容，经对比合并、去重、口径统一后形成。
...
- **ToolFacade** → Core Application Services（ContextPackService、WritingGenerationService、CandidateDraftService、AIReviewService 等）。

AgentRuntimeService 不直接调用任何 Core Application Service，必须通过 ToolFacade。

### 3.2 与 AIJobSystem 的关系

**冻结方案：AgentSession 独立建模并独立持久化，一个 AgentSession 关联一个主 AIJob（一对一），AgentStep 默认一对一映射为 AIJobStep。**

理由：

1. AgentSession 与 AIJob 的职责不同——AgentSession 表达"P1 业务编排语义"（PPAO 阶段、Agent 类型、Observation 决策），AIJob 表达"异步任务与前端轮询语义"（进度、取消、失败、重试）。
2. 若用 AIJob 直接承载 Agent 业务语义，会把 Runtime 状态、PPAO 阶段、waiting_for_user、Observation 等高层概念塞进通用任务系统，导致职责污染。
3. 一个主 AIJob 对应一个 AgentSession，最适合前端轮询、取消、失败展示和与 P0 的兼容。
...
- **PPAO 是 Agent 内部的执行机制**：每个 Agent（Memory / Planner / Writer / Reviewer / Rewriter）在 AgentRuntime 中按 PPAO 循环执行自己的步骤。PPAO 定义的是单个 Agent 内部"如何执行"的机制。
...
→ Planning
→ Action
→ waiting_observation / waiting_user（如有）
→ Observation
→ next step / completed / partial_success / failed / cancelled
```
...
- AgentSession.session_id ↔ AIJob.id 互不替代。
...
- partial_success 是否允许进入 HumanReviewGate 由 AgentOrchestrator（P1-02）判定，Runtime 不阻止。
...
**职责**：创建 AgentSession、关联主 AIJob、初始化 AgentRunContext。
...
**约束**：work_id 必须存在；caller_type 必须合法；user_instruction 脱敏后存储。

### 10.3 start_session

**职责**：将 session 从 pending 推进到 running。
...
4. 通知 AgentOrchestrator 开始编排。

### 10.4 run_next_step

**职责**：读取当前 session / context / observation，推进 PPAO，创建或更新当前 step。
...
**职责**：将 session 置为 paused，阻止新的 step 推进。

**输入**：session_id。

**行为**：
1. 校验 AgentSession.status = running 或 waiting_for_user。
2. 如果当前 AgentStep 正在执行 Tool 调用，等待当前 Tool 调用完成。
3. 当前 Tool 调用完成后，不自动启动下一个 Tool 或下一个 Step。
4. AgentSession.status = paused、paused_at = now。
5. 同步 AIJob.status = paused。

**约束**：pause 不中断正在执行的 Provider 请求（与 P0 保持一致）；pause 后迟到 ToolResult 仍正常记录，但不自动推进到下一步。

### 10.6 resume_session

**职责**：从 paused 恢复到 running。
...
**约束**：resume 不重复执行已完成的 AgentStep；如果关键状态丢失导致无法安全 resume，返回 error_code = writing_task_missing。

### 10.7 cancel_session

**职责**：将 session 从 running / waiting_for_user / paused 经 cancelling 推进到 cancelled。
...
**约束**：cancel 不删除已 succeeded 的 AgentStep、CandidateDraft、LLMCallLog / ToolAuditLog。

### 10.8 retry_session

**职责**：对 failed 或 partial_success 的 session 创建新 session 重新执行。
...
**约束**：retry_session 不修改原 AgentSession；retry_session 不删除原历史记录。

### 10.9 retry_step

**职责**：对 failed 的 step 执行受控重试。
...
**约束**：retry_step 不删除历史 attempt 和 Observation；不自动重试 formal_write / apply / memory_formalize 类操作。

### 10.10 record_observation

**职责**：写入 AgentObservation 并推进下一步状态。
...
**职责**：生成 AgentResult，将 session 标记为 completed 或 partial_success。
...
**职责**：写入错误码与脱敏错误信息，将 session 标记为 failed。
...
**职责**：将迟到结果写为 ignored_late_result，明确不推进业务状态。
...
- find_recoverable_sessions：查询服务重启后可恢复的会话（status = running / waiting_for_user）。
...
## 十三、pause / resume / cancel / retry / late result 规则

### 13.1 错误分类

| 错误类别 | 示例 error_code | 可重试 | 处理方式 |
|---|---|---|---|
| 输入/上下文错误 | input_error、perception_input_missing | 否 | AgentStep failed 或 waiting_user |
| 状态冲突 | state_conflict | 否 | AgentStep failed |
| 权限拒绝 | permission_denied、tool_permission_denied | 否 | AgentStep failed，防御性日志 |
| Tool 执行错误 | tool_execution_error | 视类型 | 见 13.2 / 13.3 |
| 输出校验错误 | validation_error、output_validation_failed | 是（受上限约束） | schema retry |
| 外部超时 | external_timeout、provider_timeout | 是（受上限约束） | Provider retry |
| 外部不可用 | external_unavailable、provider_unavailable、provider_rate_limited | 是（受上限约束） | Provider retry |
| 需要用户 | user_required | 否 | waiting_for_user |
| 会话已取消 | session_cancelled | 否 | ignored_late_result |
| 迟到结果 | late_result_ignored | 否 | 忽略并记录 |
...
- **retry_step** 是默认推荐方式：在同一个 AgentSession 内重试单个 failed Step，保留原 session 上下文。
...
- AgentOrchestrator 负责 partial_success 的业务判定。
...
- AgentSession 通过主 AIJob 提供前端轮询兼容。
...
- P1 扩展 side_effect_level（增加 plan_write、safe_write_candidate、safe_write_review、safe_write_suggestion、trace_only）。
...
5. 不实现 CandidateDraftVersion（多轮迭代）。
6. 不实现 AI Suggestion。
7. 不实现 Conflict Guard。
8. 不实现 StoryMemory Revision。
9. 不实现 AgentTrace 完整详情（只定义 trace_id 贯穿与最小事件记录）。
10. 不实现 P1 API / 前端集成。
11. 不实现 P2 自动连续续写队列。
...
- [ ] waiting_for_user 映射为 AIJob running。
- [ ] partial_success 映射为 AIJob completed。
...
P1-01 AgentRuntime 详细设计确认以下内容已冻结：

1. **PPAO 机制**：Perception → Planning → Action → Observation 是 Agent Runtime 的通用执行循环。每个 Agent 内部按 PPAO 执行，五 Agent Workflow 是 Agent 之间的编排顺序，两者正交。

2. **AgentSession 独立建模**：AgentSession 独立持久化，关联一个主 AIJob（一对一），AgentSession 是业务状态主真源，AIJob 是异步任务与前端轮询投影。

3. **AgentStep 映射 AIJobStep**：默认一对一映射，Step 内 retry 通过 attempt 机制表达，不默认新增 AIJobStep。

4. **状态机完整**：AgentSession 9 个状态（4 个终态），AgentStep 9 个状态（5 个终态），状态流转规则确定。

5. **安全边界**：ToolFacade 是唯一受控入口，caller_type = agent 不能执行 user_action 操作，formal_write 永远禁止。

6. **P0 兼容**：P0 MinimalContinuationWorkflow 保留，P0 AIJobSystem / ToolFacade / CandidateDraft / HumanReviewGate 继续复用，V1.1 Local-First 保存链路不变。

---

## 十八、P1-01 待确认点

以下问题只登记，不修改总纲。需由后续模块协同确认：

1. **AgentStep 与 AIJobStep 是否在所有场景都保持严格一一映射**：P1-01 默认一对一。如果 P1-02 AgentWorkflow 需要少量聚合 step 映射为单个 AIJobStep（或反之），需在 P1-02 中提出并回刷 P1-01。
...
6. **partial_success 是否允许直接进入 HumanReviewGate**：P1-01 建议允许（如果 Writer Agent 成功生成 CandidateDraft，即使 Reviewer Agent 失败，用户仍可查看候选稿）。最终规则由 P1-02 AgentWorkflow 和 P1-09 HumanReviewGate 协同确认。

7. **cancelling 状态是否对前端单独展示**：P1-01 建议 cancelling 作为内部过渡状态保留，前端可选择性展示。最终展示策略由 P1-11 确认。

8. **step retry 的最大次数是否统一由 Runtime 配置**：P1-01 只冻结"必须可配置"（默认 max_attempts = 3），不冻结具体数值是否由 Runtime 统一管理还是由具体 AgentWorkflow / Tool 类型决定。最终由 P1-02 / P1-03 确认。

9. **caller_type 体系是否细分**：P1-01 以 agent 为基础 caller_type（另保留 workflow_compat、system_maintenance、user_action）。如果 P1-02 / P1-03 需要更细粒度的 caller_type（如 agent_memory、agent_writer）用于权限校验，需在相应文档中提出并回刷 P1-01。

10. **P1-02 AgentOrchestrator 如何判定 partial_success / failed**：partial_success 的业务判定条件（哪些 Step 失败仍可算 partial_success，哪些必须算 failed）由 P1-02 冻结。P1-01 只提供状态能力。

11. **P1-03 五 Agent 的具体 Step 序列与 Tool 权限矩阵**：P1-01 已预留 agent_type + action 字段方向，具体 action 清单和 Tool 白名单由 P1-03 冻结。

12. **服务重启后 AgentSession 的恢复策略**：P1-01 继承 P0-02 策略——running AgentSession 标记为 paused，reason = service_restarted。是否自动 resume、是否通知用户，由 P1-11 结合前端体验确认。
...
| Agent 与 AIJobSystem 的关系 | 第三章 / 第五章：AgentSession 独立建模，一对一主 AIJob，状态映射表 |
...
1. **是否以 _001 为主稿**：是。文档结构、状态机定义、PPAO 详细设计、AgentRuntimeService 用例、Repository Ports、安全边界等核心内容以 _001 为主稿。

2. **是否吸收原始版有效内容**：是。吸收了原始版本中的 PPAO 通用 Step 类型（perceive_context 等）、Observation 类型扩展（state_change、error_event、late_result_ignored）、caller_type 扩展（workflow_compat、system_maintenance）、side_effect_level 命名优化（safe_write_candidate 等）、current_phase 字段、execution_guard_flags、next_action_hint、错误分类表、additional 待确认项。

3. **是否删除重复和冲突表述**：是。去除了原始版中 source_job_id / source_job_step_id 的前缀命名（统一为 job_id / job_step_id）；统一了 session_status → status、step_status → status 的命名；去除了两版中口径不一致的 side_effect_level 命名（原始版 formal_write_forbidden vs _001 版 formal_write，统一为 formal_write_forbidden）；合并了重复的状态机说明。

4. **是否保持 P1-01 边界，没有进入 P1-02 / P1-03 / P1-10 / P1-11**：是。AgentOrchestrator 的编排职责明确归入 P1-02；五 Agent 具体 action 清单标记为 P1-03 冻结；AgentTrace 完整模型归入 P1-10；API 与前端协议细节归入 P1-11。

5. **是否保持 P0 兼容，不推翻 P0**：是。P0 MinimalContinuationWorkflow 保留为兼容路径；P0 AIJobSystem / ToolFacade / CandidateDraft / HumanReviewGate 继续复用；V1.1 Local-First 链路不变。

6. **是否明确 AgentRuntime / AgentOrchestrator 分工**：是。第三章 3.4 节、第四章 4.2 节、第四章 4.7 节、第十四章 14.1 节多次明确：Runtime 负责 PPAO 推进，Orchestrator 负责五 Agent 编排顺序。

7. **是否明确 AgentSession / AIJob 分工**：是。第三章 3.2 节、第五章 5.5 节、第十章 10.15 节明确：AgentSession 独立建模，AIJob 不承载 Agent 业务语义；提供完整状态映射表。

8. **是否明确 ToolFacade 唯一入口**：是。第二章 2.4 节、第三章 3.3 节、第十二章 12.5 节明确：ToolFacade 是唯一受控入口，Runtime 不直接调用任何 Application Service。

9. **是否明确 HumanReviewGate / user_action / formal_write 安全边界**：是。第十二章 12.2-12.4 节明确：caller_type = agent 不能执行 user_action 操作；formal_write_forbidden 永远禁止 Agent 调用；accept / apply CandidateDraft 必须走 user_action 路径。

10. **是否闭环 pause / resume / cancel / retry / late result**：是。第十三章完整覆盖：pause → resume 路径；cancel → cancelling → cancelled 路径；cancelled 后 late result → ignored_late_result 路径；retry_step / retry_session 区别；partial_success / failed 判定；waiting_for_user 不可自动跳过。

11. **是否没有生成代码、DDL、任务拆解或开发计划**：是。文档不含任何代码、DDL、索引、迁移脚本、开发任务编号或开发计划。

...
1. `accepted` 与 `applied` 必须在作者界面使用不同中文提示：
   - accepted：已接受候选版本（尚未写入章节草稿）
   - applied：已应用到章节草稿
2. `superseded/stale` 不能只显示状态码，必须附中文解释（已有新版本/依据可能过期）。
3. 版本链是内部实现结构；作者默认看到“版本列表 + 差异 + 下一步操作”，而不是状态机图。
4. 所有门控动作仍必须 user_action，不因文案简化改变边界。
...
12. 给人用优先：按钮文案必须说明动作后果（accept=认可版本不改正文，apply=写入章节草稿，reject=仅拒绝不删除）。
13. 版本列表默认突出“当前推荐处理版本”，避免让用户在技术状态标签中自行推断下一步。

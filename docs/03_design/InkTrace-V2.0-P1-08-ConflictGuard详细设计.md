...
1. ConflictGuard 是系统检测机制；作者界面必须展示“冲突影响 + 处理动作”，不展示底层检测实现细节。
2. 严重度映射为作者语义：
   - info：提示信息
   - warning：有风险，可继续
   - blocking：必须先处理，当前不可继续
3. `blocking` 场景下必须给出可执行的下一步，不允许只返回错误码。
4. 冲突记录详情默认展示摘要与证据引用，不展示完整 Prompt / ContextPack / 原始日志。
...
9. 给人用优先：冲突卡片默认展示“是否阻止应用 + 用户下一步操作”，不得要求用户先理解 conflict_type 枚举。
10. blocking/warning/info 必须映射为中文决策文案：必须先处理 / 建议确认后继续 / 仅提示。

...
唯一正式来源：`docs/01_requirements/InkTrace-V2.0-需求规格说明书.md`

冻结裁决：自动续写统一逐章确认；每章候选稿就绪后必须等待真实用户操作，禁止自动推进下一章。P2 功能采用“系统可用性 + 作者偏好”两层开关；分析看板统一为“AI 使用分析”，不得判断正文是否由 AI 生成。所有 `*_001.md`、草稿和备份文档不得作为实现依据。
...
- Edge Case：
  - 用户离开页面：当前已在途的单章任务可安全收口，但该章候选稿就绪后必须进入 `WAITING_USER_DECISION`，不得自动生成下一章。
- Acceptance：
...
### R-AI-DRAFT-05 受控自动逐章续写队列

- Rule：自动续写必须放在 P2，且统一按逐章确认模式受控执行；不存在连续候选自动推进模式。
- Behavior：
  - 用户设定目标章数、目标字数或停止条件。
  - 系统逐章生成候选稿。
  - 每章审稿。
  - 每章可修订。
  - 每章候选稿就绪后固定进入 `WAITING_USER_DECISION`。
  - 只有真实 `caller_type=user_action`、`user_action=true` 的 `confirm-continue` 才能推进候选章间状态并生成下一章。
  - `confirm-continue` 与 CandidateDraft `apply` 是两个独立用户门；前者不代表接受或应用当前候选稿。
  - 不自动合并正式正文。
...
- Acceptance：
  ...
  - Agent、workflow、system、服务重启均不能伪造 `confirm-continue` 或自动推进下一章。
...
- Rule：P2 提供面向普通小说作者的“AI 用量与预算”和“创作分析”页面；页面先回答作者能理解的问题，技术明细默认下沉。
- Behavior：
  - AI 用量与预算先展示已知预计费用、剩余预算和保护状态；模型服务、模型名称、模型分工、AI 用量、耗时和采用状态仅在次级明细展示。
  - 创作分析展示写作统计、节奏、对白、高频词、风格一致性和 AI 使用分析。
  - AI 使用分析只展示候选稿采用次数、平均修订轮次和常见词频等可解释事实，不提供“AI 检测器”，不得判断或标注正文是否由 AI 生成。
  - 所有分析只读正式章节正文和必要元数据，不读取未应用 CandidateDraft 正文，不自动修改作品。
- Edge Case：
  - 成本估算缺少官方价格：使用用户配置单价。
  - 费用或用量无法可靠核算：显示未知并保护性暂停，不得显示为 0、免费或可继续。
  - 无风格画像：使用本地文本统计降级并明确“仅供参考”。
- Acceptance：
  - 用户可用白话查看作品级 AI 用量、预计费用、预算保护和基础创作分析。
  - 页面固定声明 AI 常见词汇只统计出现频率，不代表文本由 AI 生成。

### R-AI-ENH-06 写作助手功能开关

- Rule：普通作者只能在设置页开启系统已经允许开放的写作助手；环境变量和发行配置属于系统级熔断，不作为作者操作方式。
- Behavior：
  - 后端提供统一能力清单，返回系统可用性、作者偏好和最终有效状态。
  - 设置页使用“接着写、开篇助手、大纲辅助、选区改写、风格画像、@引用”等作者语言，不显示 Feature Flag、环境变量或内部模块名。
  - 已验收功能可开启或关闭；实验功能必须明确标注；未完成或系统禁用功能显示“尚未开放”且不能开启。
  - 作者关闭功能只隐藏入口和停止新的使用，不删除历史候选稿、设置、引用或分析数据。
  - 功能开关不得绕过 Provider 配置、预算保护、DirectionSelection、PlanConfirmation、HumanReviewGate、MemoryReviewGate 或 ConflictGuard。
- Edge Case：
  - 系统可用性与作者偏好不一致时，以系统禁用为准，并显示安全中文原因。
  - 能力清单加载失败时，P2 入口保护性关闭；P0/P1 写作与 Local-First 保存不受影响。
- Acceptance：
  - 普通作者无需脚本或环境变量即可管理已经开放的写作助手。
  - 前后端使用同一份最终有效状态，不出现“入口可见但 API 503”或“API 可用但入口隐藏”的漂移。
...
- [ ] 多章续写按章生成候选稿。
- [ ] 自动逐章续写队列具备停止条件，并在每章候选稿就绪后等待真实用户确认是否继续。
...
3. API Key 的最终存储方式。
4. 自动续写队列严格放入 P2，并统一采用逐章确认模式（已冻结）。
5. Citation Link 以候选稿元数据实现，还是作为后续正文标注能力实现。
6. @ 引用系统先做 AI 建议态，还是直接支持正文交互。
7. Agent Trace 只保存摘要、hash 和引用 ID；不得保存完整 Prompt、完整 Context Pack、完整正文、完整候选稿或 API Key（已冻结）。

...
- **不做大重构**：只删旧业务，不重构旧文件。
- **优先隔离，后删除**：优先通过路由/入口隔离旧系统（如不注册 AI Router），确保新 V1 可独立运行后，再进行物理删除。防止“删完项目起不来”的致命问题。
- **允许最小修复**：为了保证删除后项目仍可启动，允许做最小量的编译/导入修复（如移除依赖注入绑定）。
- **保留工具类**：已成熟的工具函数可复用。
...
| 1-28 自动保存链路 | **输入 → 立即写本地缓存 → debounce(2-3s) → API同步 → 成功后清理本地缓存** |

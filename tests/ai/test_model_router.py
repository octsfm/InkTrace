...
def test_model_router_resolves_runtime_role_from_author_facing_role(tmp_path) -> None:
    store = FileAISettingsStore(tmp_path / "ai_settings.json")
    store.save(
        AISettings(
            provider_configs={
            ...
            model_role_mappings={
                "analysis": ModelSelection(provider_name="fake", model_name="fake-chat")
            },
        )
    )
    registry = ProviderRegistry()
    ...
    assert router.resolve_model("outline_analyzer") == ModelSelection(
        provider_name="fake",
        model_name="fake-chat",
    )
    assert router.resolve_model("manuscript_analyzer") == ModelSelection(
        provider_name="fake",
        model_name="fake-chat",
    )
